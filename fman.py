	...
	while True:
		...
		if file.isdigit():
			...
			if file in range(len(files)):
				if fman.overwrite_save(files[file]):
...
def move_old_dailys(files):
	for f in files:
		if fman.is_daily(f.split(ext)[0]):
			fman.archive(f)
	return None
	...
	while True:
		...
		if to_delete.isdigit():
			...
			if to_delete in range(len(files)):
				fman.confirm_delete(files, to_delete)
				return None
			else:
				slowprint(invalid_fn)
